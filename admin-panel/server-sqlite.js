...
app.get('/api/customers', authenticateToken, (req, res) => {
  db.all('SELECT * FROM customers ORDER BY created_at DESC', (err, customers) => {
    ...
    // Eğer müşteri yoksa default müşteriyi ekle
    if (customers.length === 0) {
      const defaultCustomer = {
        id: 'ffeee61a-8497-4c70-857e-c8f0efb13a2a',
        display_name: 'Zerda Gold',
        email: 'admin@zerdagold.com',
        created_at: new Date().toISOString()
      };
      
      db.run(
        'INSERT OR IGNORE INTO customers (id, display_name, email, created_at) VALUES (?, ?, ?, ?)',
        [defaultCustomer.id, defaultCustomer.display_name, defaultCustomer.email, defaultCustomer.created_at],
        function(err) {
          if (err) {
            console.error('Error inserting default customer:', err);
            return res.status(500).json({ success: false, error: err.message });
          }
